...
export function ExportButton({
  ...
  function downloadPDF() {
    ...
    const summaryHtml = evaluationData?.panelSummary
      ? `<div style="margin-top:18px;padding:12px 14px;background:#f8fafc;border-left:3px solid ${categoryColour};border-radius:4px">
    ...
    const l2Html = l2Reasons.length
      ? `<div style="margin-top:18px">
           <p style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:#6b7280;margin:0 0 6px">L2 Rejection Reasons</p>
           <ul style="margin:0;padding-left:18px">
             ${l2Reasons.map(r => `<li style="font-size:11px;color:#374151;margin:2px 0">${esc(r)}</li>`).join('')}
           </ul>
         </div>`
      : '';
